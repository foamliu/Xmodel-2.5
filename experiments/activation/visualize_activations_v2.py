...
parser.add_argument('--model_path', type=str, default='/data2/liuyang/models/Xmodel2.5/random_init',
...
for token_idx in range(args.n_tokens):
    ...
    # logits 不需要缩放，因为它们已经是最终输出
    if hasattr(model.config, 'use_mup') and model.config.use_mup:
        current_embedding *= model.config.mup_input_scale
...
def compute_activation_stats(activations):
    ...
    for name, values in activations.items():
        if name.endswith('outputs'):
            stats[name] = [np.sqrt(np.mean(v**2)) for v in values]
        else:
            stats[name] = np.sqrt(np.mean(values**2))
    return stats
