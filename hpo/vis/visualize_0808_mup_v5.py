...
def visualize_hpo_results(file_path):
    ...
    plt.figure(figsize=(12, 6))
    # 绘制所有点（蓝色）
    sns.scatterplot(data=df, x='params_swiglu', y='value', color='blue', alpha=0.6)
    # 高亮最小的10个点（红色）
    sns.scatterplot(data=df[df['is_top10']], x='params_swiglu', y='value', color='red')
    plt.title('use_swiglu vs. lm loss (Top 10 lowest in red)')
    plt.xlabel('use_swiglu')
    plt.ylabel('lm loss')
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    ...
    file_path = "hpo_results/hpo_mup_v5_results_08080758.csv"
