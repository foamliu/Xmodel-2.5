...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "eta_ratio": trial.suggest_categorical('eta_ratio', [2 ** -1, 2 ** -0.5, 2 ** 0, 2 ** 0.5, 2 ** 1]),
            "num_layers": trial.suggest_categorical("num_layers", [2 ** i for i in range(2, 7)]),
        }
        ...
        lr_hidden = learning_rate / (mup_width_multiplier * math.sqrt(num_layers)) * eta_ratio
        train_iters = 500 * mup_depth_multiplier  # 根据num_layers调整训练迭代次数，使得 D=10N, 即数据量大约等于10倍参数量（嵌入层不算）。
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            ...
            "eta_ratio": [2 ** -1, 2 ** -0.5, 2 ** 0, 2 ** 0.5, 2 ** 1],
            "num_layers": [2 ** i for i in range(2, 7)]
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["eta_ratio"]) * len(
...
if __name__ == "__main__":
    ...
    learning_rate_to_be_tested = [2 ** i for i in range(-10, -5)]
