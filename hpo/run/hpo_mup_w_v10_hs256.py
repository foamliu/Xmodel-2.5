...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "global_batch_size": trial.suggest_categorical('global_batch_size', [6, 12, 24, 48, 96, 192, 240, 384, 480, 960]),
        }
        ...
        micro_batch_size = min(global_batch_size, 12)

        mup_width_multiplier = hidden_size / mup_base_width
        mup_depth_multiplier = num_layers // mup_base_depth
        batch_size_multiplier = 24.0 / global_batch_size
        num_attention_heads = hidden_size // 64
        ffn_hidden_size = hidden_size * 5 // 2

        # train_iters = round(500 * mup_width_multiplier * mup_depth_multiplier * batch_size_multiplier)      # 根据hidden_size调整训练迭代次数，使得 D=10N, 即数据量大约等于10倍参数量（嵌入层不算）。
        train_iters = round(500 * mup_width_multiplier * mup_depth_multiplier)
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            ...
            "global_batch_size": [6, 12, 24, 48, 96, 192, 240, 384, 480, 960],
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["global_batch_size"])
