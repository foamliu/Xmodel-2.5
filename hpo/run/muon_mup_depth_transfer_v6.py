...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        init_std = 0.02 * math.sqrt(mup_width_multiplier) if params["use_mup"] else 0.02

        lr_embedding = learning_rate
        lr_hidden = learning_rate / mup_width_multiplier
        train_iters = round(500 * mup_depth_multiplier * mup_width_multiplier)  # 根据num_layers调整训练迭代次数，使得 D=10N, 即数据量大约等于10倍参数量（嵌入层不算）。
        ...
        if params["use_mup"]:
            cmd += ["--use-mup",
                    "--mup-input-scale", "15.822221213310598",
                    "--mup-output-scale", "1.5119977019679969",
                    "--mup-attention-residual-scale", "7.720138434724928",
                    "--mup-ffn-residual-scale", "1.6788935011097403",
                    ]
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        study = optuna.create_study(
            direction="minimize",
            sampler=sampler,
            study_name="muon_mup_depth_transfer_v6"
        )
...
if __name__ == "__main__":
    ...
    learning_rate_range = [2 ** i for i in range(-14, -1)]
