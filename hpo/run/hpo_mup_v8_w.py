...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "global_batch_size": trial.suggest_categorical("global_batch_size", [24, 48, 120, 240, 480]),
            "muon_matched_adamw_rms": trial.suggest_float("muon_matched_adamw_rms", 0.1, 0.5),
        }
        ...
        muon_matched_adamw_rms = params["muon_matched_adamw_rms"]
        ...
        batch_size_multiplier = base_batch_size / global_batch_size
        num_attention_heads = hidden_size // 64
        ffn_hidden_size = hidden_size * 5 // 2

        train_iters = round(
            500 * mup_depth_multiplier * mup_width_multiplier * batch_size_multiplier)  # 调整训练迭代次数，使得 D=10N, 即数据量大约等于10倍参数量（嵌入层不算）。
        ...
        cmd = [
            ...
            "--optimizer", "muon",
            "--muon-matched-adamw-rms", str(muon_matched_adamw_rms),
        ]
