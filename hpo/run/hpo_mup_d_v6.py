...
class MegatronHPO:
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            ...
            "use_depth_mup": [True, False],
            "num_layers": [2 ** i for i in range(2, 7)]
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["use_depth_mup"]) * len(
...
if __name__ == "__main__":
    ...
    learning_rate_to_be_tested += [2 ** -9.5, 2 ** -9.25, 2 ** -8.75, 2 ** -8.5, 2 ** -8.25, 2 ** -7.75, 2 ** -7.5,
                                   2 ** -7.25]
