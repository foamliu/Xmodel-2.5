...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        # 超参数采样
        params = {
            "learning_rate": trial.suggest_float("learning_rate", 2 ** (-15), 2 ** (-3), log=True),
            "q_lora_rank": trial.suggest_categorical('q_lora_rank', [0, 32, 64, 128, 256]),
            "kv_lora_rank": trial.suggest_categorical("kv_lora_rank", [32, 64, 128]),
            "qk_head_dim": trial.suggest_categorical("qk_head_dim", [48, 64, 96]),
            "v_head_dim": trial.suggest_categorical("v_head_dim", [32, 64, 96]),
            "qk_pos_emb_head_dim": trial.suggest_categorical("qk_pos_emb_head_dim", [16, 32]),
        }
        ...
        qk_head_dim = params["qk_head_dim"]
        v_head_dim = params["v_head_dim"]
        qk_pos_emb_head_dim = params["qk_pos_emb_head_dim"]
        hidden_size = 1536
        ...
        if q_lora_rank > 0:
            cmd += [
                ...
                "--no-rope-fusion",
                "--q-lora-rank", str(q_lora_rank),
                "--kv-lora-rank", str(kv_lora_rank),
                "--qk-head-dim", str(qk_head_dim),
                "--qk-pos-emb-head-dim", str(qk_pos_emb_head_dim),
                "--v-head-dim", str(v_head_dim),
            ]
        else:
            cmd += [
    ...
    def _parse_validation_loss(self, log_file: str) -> float:
        ...
        elapsed_time_per_iteration = 0.0
        total_number_of_parameters_in_billions = 0.0
                ...
                if "elapsed time per iteration (ms):" in line:
                    ...
                    for part in parts:
                        if "elapsed time per iteration (ms):" in part:
                            elapsed_time = part.split(':')[1].strip()
                            elapsed_time_per_iteration = float(elapsed_time)

                if "Total number of parameters in billions:" in line:
                    # 提取模型规模
                    parts = line.split(':')
                    total_number_of_parameters_in_billions = float(parts[-1].strip())

        except ValueError as ve:
            logger.error(f"Value error while parsing log file: {ve}")
        except Exception as e:
            logger.error(f"Error parsing log file: {e}")
        return best_loss, elapsed_time_per_iteration, total_number_of_parameters_in_billions
    ...
    def _run_trial(self, trial):
        ...
        try:
            ...
            try:
                ...
                # 获取最终损失
                final_lm_loss, elapsed_time_per_iteration, total_number_of_parameters_in_billions = self._parse_validation_loss(log_file)
                if final_lm_loss is None:
                    raise RuntimeError(f"Failed to parse loss for trial {trial.number}")

                trial.set_user_attr("elapsed_time_per_iteration", elapsed_time_per_iteration)
                trial.set_user_attr("total_number_of_parameters_in_billions", total_number_of_parameters_in_billions)
