...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "decoupled_lr": trial.suggest_float("decoupled_lr", 2 ** (-15), 2 ** (-3), log=True),
            "init_std": trial.suggest_float("init_std", 0.001, 0.2),
            "mup_input_scale": trial.suggest_float("mup_input_scale", 10.0, 20.0),
            "mup_attention_residual_scale": trial.suggest_float("mup_attention_residual_scale", 2.0, 8.0),
            "mup_ffn_residual_scale": trial.suggest_float("mup_ffn_residual_scale", 1.0, 6.0),
            "swiglu": trial.suggest_categorical("swiglu", [True, False]),
        }
        ...
        decoupled_lr = params["decoupled_lr"]
        num_layers = 4
        ...
        train_iters = 500 * mup_depth_multiplier * mup_width_multiplier  # 调整训练迭代次数，使得 D=10N, 即数据量大约等于10倍参数量（嵌入层不算）。
        ...
        cmd = [
            ...
            "--rotary-base", "500000",
            # "--swiglu",
            "--init-method-std", str(params["init_std"]),
