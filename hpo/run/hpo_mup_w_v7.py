...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "hidden_size": trial.suggest_categorical("hidden_size", [64, 128, 256, 512, 1024, 2048]),
            "decay_mode": trial.suggest_categorical('decay_mode', ["--decay-all", "--no-decay-norm-bias-embed", "--no-decay-norm-bias"]),
        }
        ...
        lr_embedding = learning_rate
        lr_hidden = learning_rate / (mup_width_multiplier * math.sqrt(num_layers))
        train_iters = round(500 * mup_width_multiplier)      # 根据hidden_size调整训练迭代次数，使得 D=10N, 即数据量大约等于10倍参数量（嵌入层不算）。
        ...
        cmd = [
            ...
            "--vocab-size", "129280",
            "--eval-interval", "9999999", # 训练期间不做评估
            "--log-interval", "25",
        ...
        if params["decay_mode"] != "--decay-all":
            cmd.append(params["decay_mode"])
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            ...
            "decay_mode": ["--decay-all", "--no-decay-norm-bias-embed", "--no-decay-norm-bias"]
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["optimizer"]) * len(search_space["hidden_size"]) * len(search_space["decay_mode"])
...
if __name__ == "__main__":
    ...
    learning_rate_to_be_tested = [2 ** i for i in range(-15, -2)]
    learning_rate_to_be_tested += [2 ** -8.75, 2 ** -8.5, 2 ** -8.25, 2 ** -7.75, 2 ** -7.5, 2 ** -7.25]
