...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        # 超参数采样
        params = {
            "learning_rate": trial.suggest_float("learning_rate", 2 ** (-10), 2 ** (4), log=True),
            "use_muon": trial.suggest_categorical('use_muon', [True, False]),
            "use_mup": trial.suggest_categorical('use_mup', [True, False]),
            "init_std": trial.suggest_float("init_std", 0.001, 2.0),
            "mup_input_scale": trial.suggest_float("mup_input_scale", 0.001, 20.0, log=True),
            "mup_output_scale": trial.suggest_float("mup_output_scale", 0.001, 20.0, log=True),
            "mup_attention_residual_scale": trial.suggest_float("mup_attention_residual_scale", 0.0001, 20.0, log=True),
            "mup_ffn_residual_scale": trial.suggest_float("mup_ffn_residual_scale", 0.0001, 20.0, log=True)
        }

        # 生成10000到65535之间的随机整数
        # random_port = random.randint(10000, 65535)
        random_port = 10000 + trial.number
        ...
        lr_hidden = learning_rate / hidden_size

        use_mup = params["use_mup"]
        use_muon = params["use_muon"]
        ...
        if use_mup:
            cmd += [
                "--use-mup",
                "--mup-input-scale", str(mup_input_scale),
                "--mup-output-scale", str(mup_output_scale),
                "--mup-attention-residual-scale", str(mup_attention_residual_scale),
                "--mup-ffn-residual-scale", str(mup_ffn_residual_scale),
            ]
