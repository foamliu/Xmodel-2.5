...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "use_abs_activation": trial.suggest_categorical('use_abs_activation', [True, False]),
        ...
        # 根据参数添加mup相关选项
        if params["use_abs_activation"]:
            cmd.append("--use-abs-activation")
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            ...
            "use_abs_activation": [True, False],
            "num_layers": [2 ** i for i in range(2, 7)]
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["use_abs_activation"]) * len(search_space["num_layers"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default="hpo_results_mup_", help="Output directory for results")
