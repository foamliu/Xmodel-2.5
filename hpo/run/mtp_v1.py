...
class MegatronHPO:
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        # 超参数采样
        params = {
            "learning_rate": trial.suggest_categorical("learning_rate", learning_rate_to_be_tested),
            "mtp_num_layers": trial.suggest_categorical('mtp_num_layers', [0, 1, 2, 3]),
        ...
        mtp_num_layers = params["mtp_num_layers"]
        ...
        if mtp_num_layers > 0:
            cmd += [
                ...
                "--mtp-num-layers", str(mtp_num_layers),
            ]
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            "learning_rate": learning_rate_to_be_tested,
            "mtp_num_layers": [0, 1, 2, 3],
            "hidden_size": [256, 512, 1024, 2048]
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["mtp_num_layers"]) * len(
...
if __name__ == "__main__":
    ...
    learning_rate_to_be_tested = [2 ** i for i in range(-12, -3)]
    learning_rate_to_be_tested = sorted(learning_rate_to_be_tested)
