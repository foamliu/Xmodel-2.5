...
class MegatronHPO:
    def __init__(self, megatron_dir, data_path, output_dir, gpus_per_trial=1, gpu_pool=None):
        ...
        if gpu_pool is None:
            # 向后兼容模式
            self.gpu_pool = GPUPoolManager(range(args.start_gpu_id, args.start_gpu_id + args.jobs))
    ...
    def _build_train_command(self, trial, trial_dir):
        ...
        params = {
            ...
            "use_u_mup": trial.suggest_categorical('use_u_mup', [True, False]),
        ...
        if params["use_u_mup"]:
            lr_embedding = learning_rate * (1.0 / math.sqrt(vocab_size))
            lr_hidden = learning_rate * (1.0 / math.sqrt(hidden_size))
            init_method_std = 1.0
        else:
        ...
        cmd = [
            ...
            "--rotary-base", "500000",
            "--swiglu",
            "--init-method-std", str(init_method_std),
    ...
    def optimize(self, n_trials=50, timeout=None, n_jobs=1):
        ...
        search_space = {
            ...
            "use_u_mup": [True, False],
            "num_layers": [2 ** i for i in range(2, 7)]
        }

        n_trials = len(search_space["learning_rate"]) * len(search_space["use_u_mup"]) * len(search_space["num_layers"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-gpu-id", type=int, default=0,
                        help="GPU Id to start from (deprecated, use --gpu-pool instead)")
