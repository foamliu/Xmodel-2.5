...
class TransformerConfig(ModelParallelConfig):
    ...
    """Base width for μP (Maximal Update Parametrization) scaling. """

    mup_input_scale: float = 1.0
    ...
    mup_attention_residual_scale: float = 1.0
    """Tunable multiplier applied to the attention residual branch output before the skip-addition."""

    mup_ffn_residual_scale: float = 1.0
    """Tunable multiplier applied to the ffn residual branch output before the skip-addition."""

    use_abs_activation: bool = False
    """Whether to use abs activation functions instead of F.silu (SwiGLU) or F.gelu."""
    ...
    def __post_init__(self):
        ...
        if self.use_abs_activation:
            def abs_activation(x):
                x = torch.abs(x)
                x = x - x.mean(dim=-1, keepdim=True)
                return x
            self.activation_func = abs_activation
        ...
        if self.init_method is None:
            if self.use_mup:
                width_multiplier = self.hidden_size / self.mup_base_width
                scaled_init_method_std = self.init_method_std / math.sqrt(width_multiplier)
                self.init_method = init_method_normal(scaled_init_method_std)
            else:
        ...
        if self.use_mup and self.softmax_scale is None:
            hidden_size_per_attention_head = self.hidden_size // self.num_attention_heads
            self.softmax_scale = 1.0 / hidden_size_per_attention_head
