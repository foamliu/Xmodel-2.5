...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
        ...
        *,
        ...
        with self.bias_dropout_add_exec_handler():
            if self.config.use_mup:
                scale = self.config.mup_attention_residual_scale / math.sqrt(self.config.num_layers)
            else:
                scale = 1.0
            # attention_output_with_bias[0] 才是实际的输出张量
            attention_output_with_bias = (
                attention_output_with_bias[0] * scale,
                attention_output_with_bias[1],   # bias 保持不变
            )
    ...
    def _forward_mlp(self, pre_mlp_layernorm_output, residual):
        ...
        with self.bias_dropout_add_exec_handler():
            if self.config.use_mup:
                scale = self.config.mup_ffn_residual_scale / math.sqrt(self.config.num_layers)
            else:
                scale = 1.0
            mlp_output_with_bias = (
                mlp_output_with_bias[0] * scale,
