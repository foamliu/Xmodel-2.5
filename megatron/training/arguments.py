...
def _add_mup_args(parser):
    group = parser.add_argument_group(title='Maximal Update Parametrization (μP)',
                       description='Parameters for Maximal Update Parametrization (μP) which enables '
                       'stable training across different model widths and depths. μP ensures consistent '
                       'learning dynamics when scaling model dimensions. See https://arxiv.org/abs/2203.03466 '
                       'for theoretical background.')
    
    group.add_argument('--use-mup', action='store_true',
                       help='Enable μP scaling for model width. When True: '
                       '- Initializes weights with μP scaling rules '
                       '- Adjusts learning rates automatically for width scaling '
                       '- Required for width scaling experiments '
                       '- Typically used with models wider than baseline')

    group.add_argument('--mup-base-width', type=int, default=256,
                       help='Reference model hidden dimension where width_multiplier=1.0. '
                       'Key properties: '
                       '- Defines baseline for μP width scaling '
                       '- Actual width = base_width * width_multiplier '
                       '- Typical values: 256-1024 (matches common model sizes) '
                       '- Should match the width used during hyperparameter tuning')
    
    group.add_argument('--mup-input-scale', type=float, default=1.0,
                       help='Scaling factor for input embeddings. Controls: '
                       '- Magnitude of embedding layer outputs '
                       '- Affects signal propagation through early layers '
                       '- Typical range: 0.5-2.0 (1.0 for most cases) '
                       '- Tune if experiencing instability in early layers')
    
    group.add_argument('--mup-output-scale', type=float, default=1.0,
                       help='Scaling factor for output layer. Controls: '
                       '- Magnitude of final layer outputs before softmax '
                       '- Affects loss scale and gradient magnitudes '
                       '- Typical range: 0.5-2.0 (1.0 for most cases) '
                       '- Tune if final layer gradients are too large/small')
    
    group.add_argument('--mup-attention-residual-scale', type=float, default=1.0,
                       help='Scaling for attention residual connections. Controls: '
                       '- Balance between residual and transformed paths '
                       '- Affects gradient flow in attention blocks '
                       '- Typical range: 0.7-1.3 (1.0 for most cases) '
                       '- Tune if attention layers train unevenly')
    
    group.add_argument('--mup-ffn-residual-scale', type=float, default=1.0,
                       help='Scaling for FFN residual connections. Controls: '
                       '- Balance between residual and transformed paths '
                       '- Affects gradient flow in FFN blocks '
                       '- Typical range: 0.7-1.3 (1.0 for most cases) '
                       '- Tune if FFN layers train unevenly')
    
    group.add_argument('--use-abs-activation', action='store_true',
                       help='Use abs activation functions instead of F.silu (SwiGLU) or F.gelu. ')

    group.add_argument('--softmax-scale', type=float, default=None,
                       help='Softmax scale for attention scaling.')
    
    group.add_argument('--no-decay-norm-bias-embed', action='store_true',
                       help='Disable decay for norm, bias, and embedding parameters. ')
    
    group.add_argument('--no-decay-norm-bias', action='store_true',
                       help='Disable decay for norm and bias parameters. ')

    group.add_argument('--log-layer-grad-norms', action='store_true',
                       help='Log per-layer gradient norms for analysis.')
    group.add_argument('--grad-norm-log-interval', type=int, default=10,
                       help='Interval (in steps) for logging gradient norms.')
    return parser
